...
contract MathMastersTest is Base_Test {
    ...
    function testTopHalfSqrt() public pure {
        uint256 x = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff2a;
        assert(MathMasters.sqrt(x) == solmateSqrt(x));
    }

    // halmos --function check_testMuldWapFuzz
    function check_testMuldWapFuzz(uint256 x, uint256 y) public pure {
        if (x == 0 || y == 0 || y <= type(uint256).max / x) {
            uint256 result = MathMasters.mulWadUp(x, y);
            uint256 expected = x * y == 0 ? 0 : (x * y - 1) / 1e18 + 1;
    ...
    function testMulWadUpUnit() public {
        uint256 x = 0xac1a33ba9c2000000000000000000001;
